...
import { generateRoomBoxSummary, MAX_BOX_WIDTH_MM } from '../utils/calculator';
import { Box, Boxes, Info, ArrowRight } from 'lucide-react';
...
export const RoomBoxSchedule: React.FC<RoomBoxScheduleProps> = ({ items, selectedRoom }) => {
  ...
  const totalSemiBoxes = filtered.reduce((s, r) => s + r.semi.boxCount, 0);
  const totalFullBoxes = filtered.reduce((s, r) => s + r.full.boxCount, 0);
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-5 border-b border-slate-200">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              How many factory carcass boxes each room needs, and their Width × Height × Depth — compared for Semi Modular vs Full Modular.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-5">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="text-[11px] font-bold text-amber-700 uppercase tracking-wider">Semi Modular Boxes</div>
            <div className="text-2xl font-black text-amber-900 mt-1">{totalSemiBoxes}</div>
            <div className="text-[11px] text-amber-700 mt-0.5">Civil-built shutter/frame units carry no factory box</div>
          </div>
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
            <div className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider">Full Modular Boxes</div>
            <div className="text-2xl font-black text-emerald-900 mt-1">{totalFullBoxes}</div>
            <div className="text-[11px] text-emerald-700 mt-0.5">
              {totalFullBoxes > totalSemiBoxes
                ? `+${totalFullBoxes - totalSemiBoxes} more boxes than Semi Modular`
        ...
        <div className="mt-4 flex items-start gap-2 text-[11px] text-slate-500 bg-slate-50 border border-slate-200 rounded-lg p-3">
          ...
          <span>
            A box is a full carcass (gables + top/bottom deck + back panel). Any unit wider than{' '}
            <strong>{MAX_BOX_WIDTH_MM}mm</strong> is split into equal boxes for handling and transport — the width shown per box below is
            already divided. Semi Modular skips boxes for civil-built shutter/frame units (depth left blank); Full Modular fabricates a box
            for every unit, defaulting depth by category when left blank.
          </span>
        </div>
      </div>
          ...
          <div key={room} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 bg-slate-50 border-b border-slate-200">
              ...
              <div className="flex items-center gap-2 text-[11px] font-bold">
                <span className="px-2.5 py-1 rounded-full bg-amber-100 text-amber-800">Semi: {roomSemiTotal} boxes</span>
                <ArrowRight className="w-3 h-3 text-slate-400" />
                <span className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800">Full: {roomFullTotal} boxes</span>
              </div>
            </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50/60 text-slate-500 uppercase text-[10px] tracking-wider border-b border-slate-200">
                    ...
                    <th className="text-left px-4 py-2.5 font-bold bg-amber-50/60">Semi: Box? W×H×D</th>
                    <th className="text-center px-4 py-2.5 font-bold bg-amber-50/60">Semi Qty</th>
                    <th className="text-left px-4 py-2.5 font-bold bg-emerald-50/60">Full: W×H×D</th>
                    <th className="text-center px-4 py-2.5 font-bold bg-emerald-50/60">Full Qty</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {roomRows.map((row) => (
                    <tr key={row.itemId} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="px-4 py-2.5 text-center font-bold text-amber-800">{row.semi.boxCount || '–'}</td>
                      <td className="px-4 py-2.5">
                      ...
                      <td className="px-4 py-2.5 text-center font-bold text-emerald-800">{row.full.boxCount || '–'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
